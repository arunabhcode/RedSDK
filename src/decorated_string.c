 ...
 *
 * "Source Code" means the accompanying software in any form preferred for
 *making
 ...
 * other recipients are not authorized to possess, modify, use, or distribute
 *the
 * Source Code.
 *
 * Red.com, Inc. hereby grants Approved Recipients the rights to modify this
 * Source Code, create derivative works based on this Source Code, and
 *distribute
 * the modified/derivative works only as Binary Code in its binary form.
 *Approved
 * Recipients may not distribute the Source Code or any modification or
 *derivative
 * work of the Source Code. Redistributions of Binary Code must reproduce this
 * copyright notice, this list of conditions, and the following disclaimer in
 *the
 * documentation or other materials provided with the distribution. Red.com,
 *Inc.
 * may not be used to endorse or promote Binary Code redistributions without
 * specific prior written consent from Red.com, Inc.
 *
 * The only exception to the above licensing requirements is any recipient may
 *use,
 * copy, modify, and distribute in any format the strlcat.c and strlcpy.c
 *software
 * in accordance with the provisions required by the license associated
 *therewith;
...
static void num_to_str(
    char* dest, size_t dest_size, int32_t num, int32_t divider, int32_t digits);
static void concatenate(char* dest,
                        size_t dest_size,
                        const char* prefix,
                        const char* body,
                        const char* postfix);
static void format_imperial_dist(char* str, size_t size, int32_t dist);
static void format_metric_dist(char* str,
                               size_t size,
                               int32_t dist,
                               int32_t num_dec);
static const char* special_char_lut_1(char sym);
static const char* special_char_lut_2(const char* sym,
                                      size_t len,
                                      int32_t builtin);
...
full_abbr_pair_t full_abbr_pairs[] = {
    {"Maximum", "Max"},       {"maximum", "max"},       {"Minimum", "Min"},
    {"minimum", "min"},       {"Record", "Rec"},        {"record", "rec"},
    {"Recording", "Rec"},     {"recording", "rec"},     {"Preview", "Prev"},
    {"preview", "prev"},      {"Position", "Pos"},      {"position", "pos"},
    {"Temperature", "Temp"},  {"temperature", "temp"},  {"Integration", "Int"},
    {"integration", "int"},   {"Exposure", "Exp"},      {"exposure", "exp"},
    {"Compensation", "Comp"}, {"compensation", "comp"}, {"Motion Mount", "MM"},
    {"Absolute", "Abs"},      {"absolute", "abs"},      {"Relative", "Rel"},
    {"relative", "rel"},      {"Distance", "Dist"},     {"distance", "dist"},
    {"Frequency", "Freq"},    {"frequency", "freq"},    {"Resolution", "Res"},
    {"resolution", "res"},    {"External", "Ext"},      {"external", "ext"},
    {"Camera", "Cam"},        {"camera", "cam"},        {"Number", "No"},
    {"number", "no"},         {"Volume", "Vol"},        {"volume", "vol"},
    {"Internal", "Int"},      {"internal", "int"},      {"Version", "Ver"},
    {"version", "ver"},       {"Channel", "Ch"},        {"channel", "ch"},
    {"Project", "Proj"},      {"project", "proj"},      {"Continuous", "Cont"},
    {"continuous", "cont"},   {"Timecode", "TC"},       {"timecode", "TC"},
    {"Independent", "Indep"}, {"independent", "indep"}, {"Space", "Sp"},
    {"space", "sp"},          {"Firmware", "FW"},       {"firmware", "FW"},
    {"Default", "Def"},       {"default", "def"},       {"Alternate", "Alt"},
    {"alternate", "alt"},     {"Calibration", "Cal"},   {"calibration", "cal"},
                                      ...
                                      int32_t builtin)
{
  ...
  switch (len)
  {
    ...
    case 4:
      if (strncmp(sym, "copy", len) == 0) return builtin ? "\xa9" : "(C)";
      break;
    ...
    case 7:
      if (strncmp(sym, "redana2", len) == 0) return builtin ? "\x01" : " ANA 2";
      if (strncmp(sym, "redsub2", len) == 0) return builtin ? "\x10" : "2";
      break;
    ...
    case 10:
      if (strncmp(sym, "redformatk", len) == 0) return builtin ? "\x06" : "K";
      break;
...
static const char* special_char_lut_1_to_2(char sym)
{
  switch (sym)
  {
    case '$': return "&red1over;";

    case '&': return "&redfover;";

    case '|': return "&redsec;";

    case '{': return "&rediso;";

    case '}': return "&redformatk;";
    ...
    case '\"': return "&redana13;";

    default: return NULL;
  }
}

void decorated_string_decode2(const char* src,
                              char* dest,
                              size_t dest_len,
                              int32_t builtin)
{
    ...
    if (*src == '&')
    {
      ...
      if (*end == ';')
      {
        const char* replacement =
            special_char_lut_2(src + 1, (end - src) - 1, builtin);
        if (replacement)
        {
...
void decorated_string_create(char* dest,
                             size_t dest_size,
                             int32_t num,
                             int32_t divider,
                             int32_t digits,
...
void decorated_string_create_fps_label(char* dest,
                                       size_t dest_size,
                                       decorated_string_len_t len,
                                       int32_t fps)
{
  switch (len)
  {
...
void decorated_string_create_int_time_label(char* dest,
                                            size_t dest_size,
                                            decorated_string_len_t len,
                                            int32_t show_angle,
                                            int32_t int_time,
                                            int32_t int_time_angle,
                                            frame_processing_t frameproc,
                                            ...
                                            int32_t ae_control)
{
  ...
  if (show_angle)
  {
    if (ae_control)
    {
      decorated_string_create(
          dest, dest_size, int_time_angle, 1000, 1, "&redae; ", "&deg;");
    }
    else
    {
      decorated_string_create(
          dest, dest_size, int_time_angle, 1000, 1, "", "&deg;");
    }
  }
  else
  {
...
void decorated_string_create_red_gamma(char* dest,
                                       size_t dest_size,
                                       decorated_string_len_t len,
                                       int32_t value)
{
  switch (len)
  {
    ...
    case DECORATED_STRING_LEN_NORMAL:
      switch (value)
      {
        ...
        case GAMMACURVE_REDGAMMA2: strlcpy(dest, "REDgamma2", dest_size); break;

        case GAMMACURVE_REDGAMMA3: strlcpy(dest, "REDgamma3", dest_size); break;

        case GAMMACURVE_REDGAMMA4: strlcpy(dest, "REDgamma4", dest_size); break;

        case GAMMACURVE_ACESPROXY:
          strlcpy(dest, "ACES Proxy", dest_size);
          break;

        case GAMMACURVE_ACESCC: strlcpy(dest, "ACEScc", dest_size); break;

        case GAMMACURVE_LOG3G12: strlcpy(dest, "Log3G12", dest_size); break;

        case GAMMACURVE_BT1886: strlcpy(dest, "BT-1886", dest_size); break;

        case GAMMACURVE_HDR2084: strlcpy(dest, "HDR-2084", dest_size); break;

        case GAMMACURVE_LOG3G10: strlcpy(dest, "Log3G10", dest_size); break;

        default: strlcpy(dest, "---", dest_size); break;
      }
      break;
    ...
    case DECORATED_STRING_LEN_ABBREVIATED:
      switch (value)
      {
        case GAMMACURVE_REDLOGFILM: strlcpy(dest, "RLF", dest_size); break;

        case GAMMACURVE_REDGAMMA2: strlcpy(dest, "RG2", dest_size); break;

        case GAMMACURVE_REDGAMMA3: strlcpy(dest, "RG3", dest_size); break;

        case GAMMACURVE_REDGAMMA4: strlcpy(dest, "RG4", dest_size); break;

        case GAMMACURVE_ACESPROXY: strlcpy(dest, "ACES", dest_size); break;

        case GAMMACURVE_ACESCC: strlcpy(dest, "ACEScc", dest_size); break;

        case GAMMACURVE_LOG3G12: strlcpy(dest, "Log3G12", dest_size); break;

        case GAMMACURVE_BT1886: strlcpy(dest, "1886", dest_size); break;

        case GAMMACURVE_HDR2084: strlcpy(dest, "2084", dest_size); break;
...
void decorated_string_create_red_color(char* dest,
                                       size_t dest_size,
                                       decorated_string_len_t len,
                                       int32_t value)
{
  switch (len)
  {
    ...
    case DECORATED_STRING_LEN_NORMAL:
      switch (value)
      {
        case COLORSPACE_REDCOLOR2: strlcpy(dest, "REDcolor2", dest_size); break;

        case COLORSPACE_REDCOLOR3: strlcpy(dest, "REDcolor3", dest_size); break;

        case COLORSPACE_REDCOLOR4: strlcpy(dest, "REDcolor4", dest_size); break;
        ...
        case COLORSPACE_ACES: strlcpy(dest, "ACES", dest_size); break;

        case COLORSPACE_REC709: strlcpy(dest, "Rec. 709", dest_size); break;

        case COLORSPACE_REC2020: strlcpy(dest, "Rec. 2020", dest_size); break;
    ...
    case DECORATED_STRING_LEN_ABBREVIATED:
      switch (value)
      {
        case COLORSPACE_REDCOLOR2: strlcpy(dest, "RC2", dest_size); break;

        case COLORSPACE_REDCOLOR3: strlcpy(dest, "RC3", dest_size); break;

        case COLORSPACE_REDCOLOR4: strlcpy(dest, "RC4", dest_size); break;

        case COLORSPACE_DRAGONCOLOR: strlcpy(dest, "DC", dest_size); break;

        case COLORSPACE_DRAGONCOLOR2: strlcpy(dest, "DC2", dest_size); break;

        case COLORSPACE_ACES: strlcpy(dest, "ACES", dest_size); break;

        case COLORSPACE_REC709: strlcpy(dest, "Rec709", dest_size); break;

        case COLORSPACE_REC2020: strlcpy(dest, "Rec2020", dest_size); break;

        case COLORSPACE_RWGRGB: strlcpy(dest, "RWG", dest_size); break;
...
#define MAIN_OUTPUT_PRESET_STRING_TABLE_COUNT \
  (sizeof(main_output_preset_string_table) / sizeof(string_table_t))

void decorated_string_create_main_output_preset(char* dest,
                                                size_t dest_size,
                                                decorated_string_len_t len,
                                                int32_t value)
{
  ...
  if (value != main_output_preset_string_table[value].enum_entry ||
      (value >= string_count))
  {
    strlcpy(dest, "---", dest_size);
    return;
  ...
  {
    strlcpy(
        dest, main_output_preset_string_table[value].abbreviated, dest_size);
  }
  else
  {
...
#define DISPLAY_PRESET_STRING_TABLE_COUNT \
  (sizeof(display_preset_string_table) / sizeof(string_table_t))

void decorated_string_create_display_preset(char* dest,
                                            size_t dest_size,
                                            decorated_string_len_t len,
                                            int32_t value)
{
  ...
  if (value != display_preset_string_table[value].enum_entry ||
      (value >= string_count))
  {
    strlcpy(dest, "---", dest_size);
    return;
...
#define OUTPUT_TRANSFORM_STRING_TABLE_COUNT \
  (sizeof(output_transform_string_table) / sizeof(string_table_t))

void decorated_string_create_output_transform(char* dest,
                                              size_t dest_size,
                                              decorated_string_len_t len,
                                              int32_t value)
{
  ...
  if (value != output_transform_string_table[value].enum_entry ||
      (value >= string_count))
  {
    strlcpy(dest, "---", dest_size);
    return;
...
void decorated_string_create_iso(char* dest,
                                 size_t dest_size,
                                 decorated_string_len_t len,
                                 int32_t value,
                                 int32_t iso_pull,
...
void decorated_string_create_redcode(char* dest,
                                     size_t dest_size,
                                     decorated_string_len_t len,
                                     int32_t value)
{
...
void decorated_string_create_file_format(char* dest,
                                         size_t dest_size,
                                         decorated_string_len_t len,
                                         int32_t value)
{
  switch (len)
  {
    ...
    case DECORATED_STRING_LEN_ABBREVIATED:
      switch (value)
      {
        case 0: strlcpy(dest, "Redcode", dest_size); break;

        case 1: strlcpy(dest, "QuickTime", dest_size); break;
      }
  }
}

void decorated_string_create_aperture(char* dest,
                                      size_t dest_size,
                                      decorated_string_len_t len,
                                      int32_t value,
                                      int32_t ae_control)
{
  if (value >= 99)
  {
    ...
    {
      decorated_string_create(
          dest, dest_size, tempvalue, 1, 0, "&redae; &redfover;", "");
    }
    else
    {
      decorated_string_create(
          dest, dest_size, tempvalue, 1, 0, "&redfover;", "");
    }
  }
  else if (value > 0)
  {
    ...
    {
      decorated_string_create(
          dest, dest_size, value, 10, 1, "&redae; &redfover;", "");
    }
    else
    {
...
void decorated_string_create_power(char* dest,
                                   size_t dest_size,
                                   decorated_string_len_t len,
                                   decorated_string_color_t* color,
                                   battery_display_mode_t display_mode,
                                   int32_t power_level,
                                   int32_t runtime)
{
  if (power_level < 101)
  {
    ...
    {
      /* color is always based on run time, not percentage */
      char dummy_run_time_str[1];
      decorated_string_create_power_runtime(
          dummy_run_time_str, sizeof(dummy_run_time_str), len, color, runtime);

      decorated_string_create_power_percent(
          dest, dest_size, len, NULL, power_level);
    }
    else if (display_mode == BATTERY_DISPLAY_MODE_TOTAL_TIME)
    {
      decorated_string_create_power_runtime(
          dest, dest_size, len, color, runtime);
    }
  }
  else
  {
    /* DC Voltage */
    decorated_string_create_power_voltage(
        dest, dest_size, len, color, power_level);
  }
}

void decorated_string_create_power_voltage(char* dest,
                                           size_t dest_size,
                                           decorated_string_len_t len,
                                           decorated_string_color_t* color,
                                           int32_t voltage)
{
  ...
  else
  {
    if (color)
    {
      if (voltage < 10500 || voltage > 16800)
      {
    ...
    snprintf(dest, dest_size, "%.1fV", (float)voltage / 1000.0);
  }
}

void decorated_string_create_power_percent(char* dest,
                                           size_t dest_size,
                                           decorated_string_len_t len,
                                           decorated_string_color_t* color,
                                           int32_t percent)
...
void decorated_string_create_power_runtime(char* dest,
                                           size_t dest_size,
                                           decorated_string_len_t len,
                                           decorated_string_color_t* color,
                                           int32_t runtime)
...
void decorated_string_create_focus_dist(char* dest,
                                        size_t dest_size,
                                        decorated_string_len_t len,
                                        focus_distance_mode_t display_mode,
                                        int32_t near_dist,
                                        int32_t far_dist)
{
  ...
  if ((near_dist == far_dist && near_dist <= 0) || near_dist < 0 ||
      far_dist < 0)
  {
    strlcpy(dest, "N/A", dest_size);
    return;
  ...
  switch (display_mode)
  {
    case FOCUS_DISTANCE_MODE_METRIC:
      format_metric_dist(near_dist_str,
                         sizeof(near_dist_str),
                         near_dist,
                         (len == DECORATED_STRING_LEN_NORMAL) ? 2 : 1);
      format_metric_dist(far_dist_str,
                         sizeof(far_dist_str),
                         far_dist,
                         (len == DECORATED_STRING_LEN_NORMAL) ? 2 : 1);
      break;
...
void decorated_string_create_focal_length(char* dest,
                                          size_t dest_size,
                                          decorated_string_len_t len,
                                          int32_t value)
{
...
void decorated_string_create_media(char* dest,
                                   size_t dest_size,
                                   decorated_string_len_t len,
                                   decorated_string_color_t* color,
                                   record_mode_t record_mode,
                                   media_display_t display_mode,
                                   int32_t media_level,
                                   int32_t runtime)
{
    ...
    else
    {
      switch (display_mode)
      {
        ...
        case MEDIA_DISPLAY_TIME:
          ...
          {
            decorated_string_create(
                dest, dest_size, media_level, 1, 0, "", "%");
          }
          else
          {
            snprintf(dest, dest_size, "%d:%02d", runtime / 60, runtime % 60);
...
void decorated_string_create_color_temperature(char* dest,
                                               size_t dest_size,
                                               decorated_string_len_t len,
                                               int32_t color_temp)
{
  ...
  else
  {
    switch (len)
    {
      ...
      case DECORATED_STRING_LEN_NORMAL:
        decorated_string_create(
            dest, dest_size, color_temp, 1, 0, "", "&redkelvin;");
        break;
...
void decorated_string_create_temperature(char* dest,
                                         size_t dest_size,
                                         decorated_string_len_t len,
                                         int32_t temp)
{
...
void decorated_string_create_fan_speed(char* dest,
                                       size_t dest_size,
                                       decorated_string_len_t len,
                                       int32_t fan_speed)
{
...
void decorated_string_create_gyro_angle(char* dest,
                                        size_t dest_size,
                                        decorated_string_len_t len,
                                        int32_t angle)
{

...
static int _RCP_get_and_parse_packet_ext(tRCPParsedPacketState *packetState,
                                         char nextChar,
                                         int destructive);
...
int RCP_validateChecksum(const tRCP *pRcpData)
{
  ...
  while (!((pScanPos[-1] == RCP2_MSG_SEPARATOR_SYMBOL) &&
           (pScanPos[0] == RCP2_MSG_CKSUM_SYMBOL)))
  {
    /* End of Message? Too many characters? No way out.... */
    if (*pScanPos == RCP2_MSG_END1_SYMBOL) return RCP2_PARSER_NO_CHECKSUM;
...
int RCP_parseHeader(tRCP *pRcpData, const char *pDeviceID, char *pCmd)
{
  ...
  *pCmd = *pCmdToken;
  ...
  if (pRcpData->pTargetID)
  {
    ...
    while (*pDeviceID != '\0')
    {
      if (*pScanPos != *pDeviceID) return RCP2_PARSER_WRONG_ID;
      pScanPos++;
      pDeviceID++;
    }
  }
...
char *RCP_getNextToken(tRCP *pRcpData)
{
  ...
  if ((*pRcpData->pParserPos == RCP2_MSG_END1_SYMBOL) ||
  ...
  do
  {
    ...
    if (*pRcpData->pParserPos == RCP2_MSG_ESCAPE_SYMBOL) pRcpData->pParserPos++;
...
int RCP_buildHeader(tRCP *pRcpData,
                    char *pBuffer,
                    int bufferLength,
                    const char *pSourceID,
                    const char *pTargetID,
                    char Cmd)
{
  /* Setup RCP Structure */
  pRcpData->pBufferPos   = pBuffer;
...
/* Fill in Header into Buffer */
  ...
  if (RCP_noBufferRoom(pRcpData, 10)) return RCP2_PARSER_BUFFER_FULL;
#endif
  ...
  *pRcpData->pParserPos++ = RCP2_MSG_START_SYMBOL;
  ...
  if (pTargetID && *pTargetID)
  {
    ...
    *pRcpData->pParserPos++ = RCP2_MSG_TARGET_SYMBOL;
    /* Add Target ID */
    pRcpData->pParserPos = RCP_copyStringChar(
        pRcpData->pParserPos, pTargetID, pBuffer + bufferLength);
  }
...
/* Add Source Marker */
  ...
  *pRcpData->pParserPos++ = RCP2_MSG_SOURCE_SYMBOL;
  /* Add Target ID */
  pRcpData->pParserPos = RCP_copyStringChar(
      pRcpData->pParserPos, pSourceID, pBuffer + bufferLength);

/* Add Command */
#ifdef RCP2_CHECK_BUFFER_OVERFLOW
  /* Check if we have room for the header */
  if (RCP_noBufferRoom(pRcpData, 2)) return RCP2_PARSER_BUFFER_FULL;
#endif
...
int RCP_addToken(tRCP *pRcpData, const char *pToken)
{
  ...
  {
    /* Deal with Escape Characters */
    if ((*pToken == RCP2_MSG_START_SYMBOL) ||
        ...
        (*pToken == RCP2_MSG_ESCAPE_SYMBOL))
    {
      ...
      if (RCP_noBufferRoom(pRcpData, 1)) return RCP2_PARSER_BUFFER_FULL;
#endif
...
int RCP_addChecksum(tRCP *pRcpData)
{
  ...
  if (RCP_noBufferRoom(pRcpData, 3)) return RCP2_PARSER_BUFFER_FULL;
#endif
...
int RCP_finalizeMessage(tRCP *pRcpData)
{
  ...
  *(pRcpData->pParserPos++) = '\0';
  ...
  /* Return final size */
  return (int)((pRcpData->pParserPos - 1) -
               pRcpData->pBufferPos); /* While calculating size, do not include
                                         '\0' at the end of RCP message. */
}

/* Wrapper Function */
/* ---------------- */

int RCP_buildMessage(char *pBuffer,
                     int maxSize,
                     const char *pSource,
                     const char *pTarget,
                     char cmd,
                     ...
                     char addChecksum)
{
  ...
  returnVal =
      RCP_buildHeader(&rcpBuffer, pBuffer, maxSize, pSource, pTarget, cmd);
  if (returnVal != RCP2_PARSER_OK) return returnVal;
...
int RCP_parseMessage(char *pBuffer,
                     int bufferSize,
...
static int RCP_noBufferRoom(const tRCP *pRcpData, int increase)
{
  if (((pRcpData->pParserPos - pRcpData->pBufferPos) + increase) >
      pRcpData->bufferLength)
    return 1;
  else
    return 0;
}
...
const char *RCP_get_packet(tRCPPacketState *packetState,
                           char nextChar,
                           int *numBytes)
{
  if (!packetState)
      ...
      if (packetState->in_packet)
      {
        ...
        switch (nextChar)
        {
          ...
          case RCP2_MSG_SEPARATOR_SYMBOL:
            ...
            {
              packetState->parsed.pParam =
                  &(packetState->buf[packetState->len]);
            }
            ...
            else if (!packetState->parsed.pExtra)
            {
              ...
              if (packetState->is_binary)
              {
                int digits              = 0;
                packetState->skip_bytes = 0;
                while ((packetState->parsed.pArg[digits] >= '0') &&
                       (packetState->parsed.pArg[digits] <= '9'))
                {
                  packetState->skip_bytes *= 10;
                  packetState->skip_bytes +=
                      packetState->parsed.pArg[digits] - '0';
                  digits++;
                }
              }
              break;
...
int RCP_strcmp(const char *s1, const char *s2)
{
  for (;;)
  {
    if ((!*s1 || *s1 == RCP2_MSG_SEPARATOR_SYMBOL ||
         *s1 == RCP2_MSG_SOURCE_SYMBOL) &&
        (!*s2 || *s2 == RCP2_MSG_SEPARATOR_SYMBOL ||
         *s2 == RCP2_MSG_SOURCE_SYMBOL))
    {
      return 0;
    }
...
size_t RCP_strlcpy(char *dst, const char *src, size_t siz)
{
  ...
  if (n != 0)
  {
    while (--n != 0)
    {
      if (*s == '\0' || *s == RCP2_MSG_SEPARATOR_SYMBOL ||
          *s == RCP2_MSG_SOURCE_SYMBOL)
      {
        *d++ = '\0';
        break;
      }
  ...
  if (n == 0)
  {
    ...
    while (*s != '\0' && *s != RCP2_MSG_SEPARATOR_SYMBOL &&
           *s != RCP2_MSG_SOURCE_SYMBOL)
    {
      if (*s == RCP2_MSG_ESCAPE_SYMBOL)
      {
...
int RCP_get_and_parse_packet_non_destructive(tRCPParsedPacketState *packetState,
                                             char nextChar)
{
  ...
  switch (packetState->cur_state)
  {
    ...
    case RCP2_STATE_ARG:
      if (delim_separator)
      {
        ...
        {
          /* This is the last allowed argument, don't try to set the next
           * argument
           * pointer. */
        }
        ...
        else
        {
          ...
          packetState->parsed.argv[packetState->parsed.argc] =
              &packetState->buf[packetState->len];
        }
      }
      else if (delim_start)
  ...
  while (packetState->cur_state != next_state)
  {
    ...
    switch (packetState->cur_state)
    {
      ...
      case RCP2_STATE_END:
        ...
        {
          if (packetState->checksum !=
              ascii2hex16b(packetState->parsed.pChecksum))
          {
        ...
        {
          if (!packetState->parsed.pCmd ||
              packetState->parsed.pCmd[0] == RCP2_MSG_SEPARATOR_SYMBOL)
          {
            packetState->last_error = RCP2_PARSER_NO_COMMAND;
            break;
          }
        ...
        {
          if (!packetState->parsed.pParam ||
              packetState->parsed.pParam[0] == RCP2_MSG_SEPARATOR_SYMBOL)
          {
            packetState->last_error = RCP2_PARSER_NO_PARAMETER;
            break;
          }

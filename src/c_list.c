...
#ifdef CLIST_USE_THREADX
#define MAX_TOTAL_LIST_ELEMENTS 25000
static TX_BLOCK_POOL m_block_pool;
static char m_buffer[MAX_TOTAL_LIST_ELEMENTS *
                     (sizeof(c_list_entry_t) + sizeof(user_defined_t))];
static int m_block_pool_initialized = 0;
#endif
#define ESCAPE_CHAR '%'
...
const char *c_list_strings_to_compress[C_LIST_NUM_STRINGS_TO_COMPRESS] = {
    "&redformatk; ", "&redformatk;", "&redkelvin;", "&redana125;", "&red1over;",
    "&redana13;",    "&redcheck;",   "&redfover;",  "&redana2;",   "&redsub2;",
    "&redfps;",      "&rediso;",     "&redsec;",    "&redae;",     "&redav;",
    "&trade;",       "&copy;",       "&amp;",       "&deg;",       "&reg;",
    "2%:1",          "3%:2",         "6%:5",        "4%:1",        "8%:1",
    "5%:4",          "4%:3",         "5.5",         "4.5",         "3.5",
    "2.5",           "HD",           "WS"};
...
#define MAX_DICTIONARY_STRING_LENGTH                                           \
  (1 + (MAX_UNCOMPRESSED_STRING_LENGTH + 1 + 1 + 1) * MAX_COMPRESSED_STRINGS + \
   1 + 1)
...
static c_list_entry_t *_c_list_get_entry_by_index(const c_list_t *c_list,
                                                  size_t idx);
static int _c_list_convert_string_token(char *pBuffer,
                                        const char *pToken,
                                        size_t maxLength,
                                        const char *characters_to_compress);
static const char _c_list_get_compressed_character(
    c_list_compressed_string_dictionary_entry_t *compressed_string_dictionary,
    size_t *compressed_character_count,
    const char *src,
    size_t src_size,
    const unsigned char *used_characters,
    char *current_unused_character);
static char _c_list_create_compressed_character(
    const unsigned char *used_characters, char *current_unused_character);
static void _c_list_compress_string_token(
    c_list_compressed_string_dictionary_entry_t *compressed_string_dictionary,
    size_t *compressed_string_count,
    char *pBuffer,
    const char *pToken,
    size_t maxLength,
    const unsigned char *used_characters,
    char *current_unused_character);
static size_t _c_list_get_token(const char *pBuffer,
                                char *pToken,
                                size_t maxLength,
                                int remove_escape_characters);
static size_t _c_list_remove_escape_characters(const char *src,
                                               char *dest,
                                               size_t dest_length);
static c_list_error_t _c_list_import_normal_string(c_list_t *c_list,
                                                   const char *str,
                                                   int compress);
static c_list_error_t _c_list_import_min_max_step_string(c_list_t *c_list,
                                                         const char *str);
static c_list_error_t _c_list_export_min_max_step_string(const c_list_t *c_list,
                                                         char *str,
                                                         size_t size);
static void _c_list_parse_compressed_string_dictionary(
    c_list_compressed_string_dictionary_entry_t *compressed_string_dictionary,
    size_t *compressed_string_count,
    const char *str_dictionary);
static void _c_list_get_used_characters(const c_list_t *c_list,
                                        unsigned char *used_characters);
static c_list_error_t _c_list_export_normal_string(const c_list_t *c_list,
                                                   char *str,
                                                   size_t size,
                                                   int compress);
static c_list_entry_t *_c_list_entry_create(const c_list_t *c_list,
                                            c_list_num_t num,
                                            const char *str,
                                            user_defined_t user_defined);
static c_list_error_t _c_list_entry_delete(const c_list_t *c_list,
                                           c_list_entry_t *entry);
static c_list_error_t _c_list_entry_copy(c_list_entry_t *dest,
                                         const c_list_entry_t *src);
                                            ...
                                            user_defined_t user_defined)
{
  ...
  if (!m_block_pool_initialized)
  {
    const UINT res = tx_block_pool_create(&m_block_pool,
                                          "cList Block Pool",
                                          sizeof(c_list_entry_t),
                                          m_buffer,
                                          sizeof(m_buffer));
    if (res != TX_SUCCESS)
    {
    ...
    m_block_pool_initialized = 1;
  }

  res = tx_block_allocate(&m_block_pool, (VOID **)&c_list_entry, TX_NO_WAIT);
  if (res != TX_SUCCESS)
  {
#ifdef CLIST_USE_LOGGER
...
#else
  if (TX_SUCCESS != tx_block_release((void *)entry))
  {
...
c_list_error_t c_list_append(c_list_t *c_list,
                             c_list_num_t num,
                             const char *str,
                             user_defined_t user_defined)
{
...
c_list_error_t c_list_insert_in_sort_order(c_list_t *c_list,
                                           c_list_num_t num,
                                           const char *str,
                                           user_defined_t user_defined)
{
                                                  ...
                                                  user_defined_t user_defined)
{
  ...
  if (C_LIST_SUCCESS ==
      c_list_find_num(c_list, num, &list_entry, C_LIST_FIND_EXACT))
  {
...
c_list_error_t c_list_insert_in_alphabetical_order(c_list_t *c_list,
                                                   c_list_num_t num,
                                                   const char *str,
                                                   user_defined_t user_defined)
{
...
c_list_error_t c_list_get_entry(const c_list_t *c_list,
                                size_t idx,
...
c_list_error_t c_list_get_current_entry(const c_list_t *c_list,
                                        c_list_entry_t *c_list_entry)
{
  c_list_error_t err;
  if (!c_list || !c_list_entry)
                               ...
                               c_list_find_t find)
{
  ...
  switch (find)
  {
    case C_LIST_FIND_EXACT: return C_LIST_FIND_ERROR;
                                   ...
                                   const char *prefix)
{
  ...
  if (prefix)
  {
    strlcpy(c_list->step_options.prefix,
            prefix,
            sizeof(c_list->step_options.prefix));
  }
  else
  {
    strlcpy(
        c_list->step_options.prefix, "", sizeof(c_list->step_options.prefix));
  }
  if (postfix)
  {
    strlcpy(c_list->step_options.postfix,
            postfix,
            sizeof(c_list->step_options.postfix));
  }
  else
  {
    strlcpy(
        c_list->step_options.postfix, "", sizeof(c_list->step_options.postfix));
  }
  ...
  for (ii = min; ii <= max; ii += step)
  {
    ...
    err = c_list_append(
        c_list, listEntry.num, listEntry.str, listEntry.user_defined);
    if (C_LIST_SUCCESS != err)
    {
      return err;
    }
...
const char _c_list_get_compressed_character(
    c_list_compressed_string_dictionary_entry_t *compressed_string_dictionary,
    size_t *compressed_string_count,
    const char *src,
    size_t src_size,
    ...
    char *current_unused_character)
{
  ...
  tmp = _c_list_create_compressed_character(used_characters,
                                            current_unused_character);
  if (tmp)
  {
    (*compressed_string_count)++;
    strncpy(
        compressed_string_dictionary[idx].uncompressed_string, src, src_size);
...
char _c_list_create_compressed_character(const unsigned char *used_characters,
                                         char *current_unused_character)
{
  ...
  while (*current_unused_character <= LAST_USABLE_CHARACTER_FOR_COMPRESSION)
  {
     ...
     * */
    if (*current_unused_character !=
        COMPRESSED_STRING_DICTIONARY_SEPARATOR_CHAR)
    {
      if (!used_characters[*current_unused_character -
                           FIRST_USABLE_CHARACER_FOR_COMPRESSION])
      {
    ...
    char *current_unused_character)
{
  ...
  while (dest_size != 1 && *src)
  {
    ...
    {
      for (ii = 0; ii < sizeof(c_list_strings_to_compress) /
                            sizeof(c_list_strings_to_compress[0]);
           ii++)
      {
        ...
        if (strncmp(src, c_list_strings_to_compress[ii], length) == 0)
        {
          ...
          const char compressed_character =
              _c_list_get_compressed_character(compressed_string_dictionary,
                                               compressed_string_count,
                                               c_list_strings_to_compress[ii],
                                               length,
                                               used_characters,
                                               current_unused_character);
...
c_list_error_t c_list_import_from_string(c_list_t *c_list, const char *str)
{
  ...
  switch (type)
  {
    ...
    default: return C_LIST_PARAM_ERROR;
  }
}
...
c_list_error_t c_list_export_to_string_ext(const c_list_t *c_list,
                                           char *str,
                                           size_t size,
                                        ...
                                        unsigned char *used_characters)
{
  ...
  while (cur_entry)
  {
    ...
    while (*tmp != '\0')
    {
      ...
      if (*tmp >= FIRST_USABLE_CHARACER_FOR_COMPRESSION &&
          *tmp <= LAST_USABLE_CHARACTER_FOR_COMPRESSION)
      {
                                                   ...
                                                   int compress)
{
  size_t cur;
  int cur_int;
  char buff[2 * C_LIST_MAX_STRING_LEN +
            1]; /* we use 2*C_LIST_MAX_STRING_LEN in case each char needs to be
                   escaped */
  const char separator[2] = {SEPARATOR_CHAR, '\0'};
  size_t ii;
  c_list_compressed_string_dictionary_entry_t
      compressed_string_dictionary[MAX_COMPRESSED_STRINGS] = {0};
  const c_list_entry_t *cur_entry;
  size_t offset = 0;
  unsigned char used_characters[LAST_USABLE_CHARACTER_FOR_COMPRESSION -
                                FIRST_USABLE_CHARACER_FOR_COMPRESSION + 1] = {
      0};
  ...
  while (cur_entry)
  {
    ...
    if (compress)
    {
      ...
      const char characters_to_compress_1[]    = {
          SEPARATOR_CHAR,
          ESCAPE_CHAR,
          COMPRESSED_STRING_DICTIONARY_START_CHAR,
          COMPRESSED_STRING_DICTIONARY_END_CHAR,
          COMPRESSED_STRING_DICTIONARY_SEPARATOR_CHAR,
          '\0'};
      const char characters_to_compress_2[] = {
          SEPARATOR_CHAR,
          ESCAPE_CHAR,
          COMPRESSED_STRING_DICTIONARY_START_CHAR,
          '\0'};
      ...
      if (!_c_list_convert_string_token(
              tmp, cur_entry->str, sizeof(tmp), characters_to_compress_1))
      {
        return C_LIST_MEM_ERROR;
      }
       ...
       * stringified list */
      _c_list_compress_string_token(compressed_string_dictionary,
                                    &compressed_string_count,
                                    buff,
                                    tmp,
                                    sizeof(buff),
      ...
      if (!_c_list_convert_string_token(
              buff, tmp2, sizeof(buff), characters_to_compress_2))
      {
        return C_LIST_MEM_ERROR;
      }
    }
    else
    {
      ...
      if (!_c_list_convert_string_token(
              buff, cur_entry->str, sizeof(buff), characters_to_compress))
      {
        return C_LIST_MEM_ERROR;
      }
    }
  ...
  if (compress)
  {
    ...
    for (ii = 0; ii < compressed_string_count; ii++)
    {
      const char dictionary_separator[] = {
          COMPRESSED_STRING_DICTIONARY_SEPARATOR_CHAR, '\0'};

      /* Strings in the dictionary were already escaped when they were
       * added, they should not be escaped again */
      offset += strlcat(str + offset,
                        compressed_string_dictionary[ii].uncompressed_string,
                        size - offset);
      if (size <= offset)
      {
        return C_LIST_BUFFER_FULL_ERROR;
                                                         ...
                                                         size_t size)
{
  ...
  /* Add Prefix */
  if (!_c_list_convert_string_token(buff,
                                    c_list->step_options.prefix,
                                    sizeof(buff),
                                    characters_to_compress))
  {
    return C_LIST_MEM_ERROR;
  }
  if (size <= strlcat(str, buff, size))
...
void _c_list_parse_compressed_string_dictionary(
    c_list_compressed_string_dictionary_entry_t *compressed_string_dictionary,
    size_t *compressed_string_count,
    const char *str_dictionary)
{
  ...
  while (*compressed_string_count < MAX_COMPRESSED_STRINGS)
  {
    ...
    const size_t len =
        _c_list_get_token(str_dictionary, token, sizeof(token), 0);
    if (len == 0)
    {
      break;
    }
    ...
    while (token[src_index] != '\0')
    {
      if (token[src_index] != ESCAPE_CHAR)
      {
        ...
        {
          compressed_string_dictionary[*compressed_string_count]
              .uncompressed_string[uncompressed_string_index] = '\0';
          src_index++;
          compressed_string_dictionary[*compressed_string_count]
              .compressed_character = token[src_index];
          break;
        }
        else
        {
          compressed_string_dictionary[*compressed_string_count]
              .uncompressed_string[uncompressed_string_index++] =
              token[src_index++];
        }
      }
      else
                                                   ...
                                                   int compress)
{
  int idx;
  size_t len;
  char buffer1[2 * C_LIST_MAX_STRING_LEN +
               1]; /* we use 2*C_LIST_MAX_STRING_LEN in case each char needs to
                      be escaped */
  c_list_entry_t listEntry;
  c_list_error_t err;
  c_list_compressed_string_dictionary_entry_t
  ...
  if (compress)
  {
    ...
    while (str[ii] != '\0')
    {
      ...
      else if (dictionary_start != -1 &&
               str[ii] == COMPRESSED_STRING_DICTIONARY_END_CHAR)
      {
        dictionary_end = ii - 2;
        break;
      }
    ...
    if (dictionary_start > 0 && dictionary_end > 0 &&
        dictionary_start < dictionary_end)
    {
      ...
      if (str_dictionary_size <= MAX_DICTIONARY_STRING_LENGTH)
      {
        ...
        _c_list_parse_compressed_string_dictionary(compressed_string_dictionary,
                                                   &compressed_string_count,
                                                   str_dictionary);
      }
      else
    ...
    if (compress)
    {
      ...
      while (src_index < len)
      {
        ...
        for (ii = 0; ii < compressed_string_count; ii++)
        {
          ...
          if (buffer1[src_index] ==
              compressed_string_dictionary[ii].compressed_character)
          {
            /* Remove any escape characters that were added
             * to the dictionary */
            char tmp[MAX_UNCOMPRESSED_STRING_LENGTH] = {0};
            _c_list_remove_escape_characters(
                compressed_string_dictionary[ii].uncompressed_string,
                tmp,
                sizeof(tmp));
                                                         ...
                                                         const char *str)
{
  ...
  /* Get current index */
  len = _c_list_get_token(str, buffer1, sizeof(buffer1), 1);
  if (len == 0) return C_LIST_NO_DATA_ERROR;
  str = str + len + 1;
  idx = atoi(buffer1);
  ...
  err = c_list_fill_generic(
      c_list, min, max, step, divider, precision, postfix, prefix);
  if (err != C_LIST_SUCCESS)
  {
    return err;
  }

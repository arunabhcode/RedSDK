...
class DebayerOpenCLJob
{
 ...
 public:
  // returns the size in bytes of the result frame that the debayer job passed
  // in will produce.
  ...
  // This call will block until the debayer on the queue executes, if the
  // debayer has already executed no blocking will occur
  // This function must be called at a point after passing the debayerOpenCLJob
  // into REDCL::processAsync
  // This will automatically be called upon destruction the DebayerOpenCLJob if
  // it is not manually called earlier
  void completeAsync();
  ...
  //
  // The user can also optionally use a cl_image object to gain more control
  // over thier output pixel format. See the CLIMAGE_OUTPUT preprocessor define
  // in the decode sample for more details.
...
class REDCL
{
 public:
  enum Status
  {
    ...
    // mode value passed in is not compatible with this SDK or the mode used
    // with the DecodeForGpuSdk call is not compatible
    Status_InvalidJobParameter_mode = 3,  // mode is not compatible with this
                                          // SDK or the mode used with the
                                          // DecodeForGpuSdk call
  ...
  // opencl_api_functions must be bound to your desired implementation of
  // OpenCL.
  // compiledKernelCacheFolder is a folder path to where you want us to store
  // and load our compiled kernel files on disk. - if you provide "" caching of
  // compiled kernels will be disabled.
  REDCL(EXT_OCLAPI_1_1 &opencl_api_functions,
        const char *compiledKernelCacheFolder);

  ~REDCL();

  // checks to see if a context and command queue are compatible with REDCL
  // This call may take several seconds on slower cards
  Status checkCompatibility(cl_context context,
                            cl_command_queue queue,
                            cl_int &clerror);

  // processes a frame.
  // Thread safe blocking call
  // This function will enqueue the opencl commands and wait for the processing
  // to complete
  //  to ensure memory buffers are not released prior to completion of kernel
  Status process(cl_context context,
                 cl_command_queue queue,
                 DebayerOpenCLJob *debayerJob,
                 cl_int &clerror);

  // enqueues all red processing on the current command queue
  // once processAsync has been called you will need to call
  // debayerJob->completeAsync
  //      This will wait for commands enqueued by this sdk for the current frame
  //      to complete and then releases any additional resources.
  //      If the commands have already completed there is no wait involved
  Status processAsync(cl_context context,
                      cl_command_queue queue,
                      DebayerOpenCLJob *debayerJob,
                      cl_int &clerror);

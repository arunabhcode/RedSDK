...
#include "red/R3DSDKDefinitions.h"

#include "red/R3DSDKMetadata.h"

#include "red/R3DSDKRocket.h"

namespace R3DSDK
{
// Bit flags that can be passed to InitializeSdk() through the
// optional_components parameter
...
InitializeStatus InitializeSdk(const char *pathToDynamicLibraries,
...
// libraries succesfully loaded. Should be called after InitializedSdk(), but
// will
...
class Clip
{
 public:
  ...
  // path when you construct the class. Load status can be checked through
  // Status().
  ...
  typedef bool (*TrimCallback)(CreateStatus status,
                               void *privateData,
                               size_t frameNoDone,
                               size_t lastFrameToDo);
  ...
  static CreateStatus CreateTrimFrom(const Clip &source,
  ...
  // Create single frame clip from another one. 'destination' is a full UTF-8
  // path
  ...
  static CreateStatus CreateSnapshotFrom(const Clip &source,
                                         const char *destination,
                                         size_t frameNo);

  // If you try to include a frame (either through CreateTrimFrom or
  // CreateSnapshotFrom)
  // that is a dropped frame you will receive CSFailedToGetSourceFrame return
  // code.
  // In this case CreateSnapshotFrom will not have created an output file at all
  // In this case CreateTrimFrom will have created a clip up to the dropped
  // frame.
  ...
  DecodeStatus DecodeVideoFrame(size_t videoFrameNo,
                                const VideoDecodeJob &decodeJob) const;
  ...
  DecodeStatus DecodeAudioBlock(size_t audioBlockNo,
                                void *outputBuffer,
  ...
  // buffer must be at least: align_to_512(numberOfSamples * numberOfChannels *
  // 4)
  // 0 <= startSample < AudioSampleCount()
  DecodeStatus DecodeAudio(unsigned long long startSample,
  ...
  DecodeStatus VideoTrackDecodeFrame(size_t videoTrackNo,
                                     size_t videoFrameNo,
                                     const VideoDecodeJob &decodeJob) const;
  ...
  // Get either the clip image processing settings or overriding RMD sidecar
  // metadata
  // (display this in your UI as the clip defaults!). Use
  // GetClipImageProcessingSettings()
  // function below to always get the clip settings. PERFORMANCE WARNING: This
  // function
  // will check for, and parse the RMD sidecar, EVERY time this function gets
  // called!
  void GetDefaultImageProcessingSettings(
      ImageProcessingSettings &settingsToFill) const;

  // Get the image processing settings as they were set on camera when the clip
  // was
  // recorded. Use this to offer "reset to clip/recorded metadata" in your
  // application.
  void GetClipImageProcessingSettings(
      ImageProcessingSettings &settingsToFill) const;

  // Create or update existing RMD sidecar file. Returns true if success, false
  // otherwise
  // The ImageProcessingSettings struct *must* have Version set to ColorVersion2
  // or above
  // for this call to succeed. ColorVersion1 is not supported within RMD sidecar
  // files.
  // The RMD sidecar file is an XML based format that travels with the clips. In
  // the future
  ...
  // Returns blend settings OR track number to decode. If an error occurs, or no
  // HDR
  // processing settings exist this function will return track 0 (A / main
  // track)
  HdrMode GetRmdHdrProcessingSettings(HdrProcessingSettings &settingsToFill,
                                      size_t &trackNoToFill) const;
  ...
  bool CreateOrUpdateRmd(HdrMode mode,
                         const HdrProcessingSettings &settingsToWrite,
                         size_t trackNoToWrite) const;

  // Whitebalance: convert a pixel area to Kelvin & Tint
  // End-user can pick a point on the image for a given frame. X and Y are in
  // 0.0 - 1.0
  // range and go from upper left (0,0) to lower right (1,1). Function fails if
  // x, y or
  // videoFrameNo are out of bounds, clip has not been loaded or frame is
  // corrupt.
  // It will only update Kelvin & Tint for the supplied ImageProcessingSettings,
  // nothing else
  DecodeStatus GetWhitebalance(ImageProcessingSettings &settingsToFill,
                               size_t videoFrameNo,
                               float x,
                               float y) const;

  // Minimum color version of the ImageProcessingSettings structure required for
  // the clip.
  // If the version is set lower in ImageProcessingSettings than the value
  // returned by this
  // function it will be changed to the highest color version available. This
  // function can
  // help to identify older footage that you may want to enable older image
  // processing for
  // to match how the image used to look in an older SDK (ie, backwards
  // compatibility).
  ColorVersion MinimumColorVersion() const;

  // Returns Default Color Version for clip. ColorVersion3 for clips recorded
  // in-camera
  ...
  // Load Creative 3D LUT (.cube only!) for use in IPP2 decoding. Load search
  // sequence:
  ...
  // If the 3D LUT is a sidecar file then it's important to only supply the
  // filename and not
  // the full path so it can later be saved correctly to the sidecar RMD file
  // when CreateOrUpdateRmd()
  ...
  DecodeStatus GetFrameMetadata(Metadata &metadataToFill,
                                size_t videoFrameNo) const;
  ...
  DecodeStatus GetFrameMetadata(Metadata &metadataToFill,
                                size_t videoFrameNo,
                                Metadata *externalMetadataToFill,
                                bool *externalDifferent) const;

  // Functions to retrieve clip (first frame) metadata through the metadata
  // database
  size_t MetadataCount() const;  // number of metadate items in the store
  bool MetadataExists(
      const char *key) const;  // check if a metadata item exists in the store
  std::string MetadataItemKey(size_t index) const;  // get key for an index

  // keep in mind that there is no guarantee which metadata item will be at
  // which
  // index. The index option is only there to allow enumeration through the
  // database.
  unsigned int MetadataItemAsInt(const char *key)
      const;  // retrieve metadata item by key and return as int
  unsigned int MetadataItemAsInt(
      size_t index) const;  // retrieve metadata item by index and return as int
  std::string MetadataItemAsString(const char *key)
      const;  // retrieve metadata item by key and return as STL string
  std::string MetadataItemAsString(size_t index)
      const;  // retrieve metadata item by index and return as STL string
  float MetadataItemAsFloat(const char *key)
      const;  // retrieve metadata item by key and return as float
  float MetadataItemAsFloat(size_t index)
      const;  // retrieve metadata item by index and return as float

  MetadataType MetadataItemType(
      const char *key) const;  // retrieve metadata item type by key
  MetadataType MetadataItemType(
      size_t index) const;  // retrieve metadata item type by index
...
struct AsyncDecompressJob
{
 ...
 public:
  // Set this flag to true if you want to abort processing this frame as soon as
  // possible
  bool AbortDecode;
  ...
  size_t VideoTrackNo;  // 0 = main (A) track, 1 = EPIC/Scarlet-X higlight
                        // protection track 2 (X track)

  size_t VideoFrameNo;

  // item is the pointer to the original item as submitted in the Decode()
  typedef void (*AsyncDecompressCallback)(AsyncDecompressJob *item,
  ...
  // SPEND AS LITTLE TIME AS POSSIBLE IN THE CALLBACK TO NOT HOLD UP PROCESSING
  // OTHER FRAMES!
  AsyncDecompressCallback Callback;
  void *PrivateData;  // available for your use as you see fit, R3D SDK will not
                      // touch this variable!
  ...
  // Size ouf the output buffer. If this value is too small the decode will
  // fail.
...
// Class for asynchronously decompressing a frame for further processing on the
// GPU.
...
// 	2) try to load as a full path
// 	3) try to load from current directory
// If the 3D LUT is a sidecar file then it's important to call
// Clip::LoadCreativeLut()
// version of this API instead! See description of that API to get more
// information.
CreativeLutHandle LoadCreativeLut(const char *lutPath,
                                  const char *clipPath = NULL);

// Unload creative 3D LUTs loaded through one of the LoadCreativeLut() APIs. The
// supplied
// handle will be set to NULL for you. Can be safely called on R3D/RMD auto
// loaded 3D LUTs.
void UnloadCreativeLut(CreativeLutHandle *handle);

// Get the path for the given Creative 3D LUT. Returns NULL if handle is
// invalid.
...
// The Output Transform takes the RWG/Log3G10 to the final output image via the
// user selected settings.
// Returns CSDone if succesful, otherwise returns error code. The following
// parameters are used:
// - utf8OutputPath  : UTF-8 path to folder where LUT will be created in.
// Existing LUT with same name will be overwritten!
...
CreateStatus SaveRWGLog3G10ToOutputTransform3DLut(const char *utf8OutputPath,
                                                  int edgeLength,
                                                  ImageColorSpace colorSpace,
                                                  ImageGammaCurve gammaCurve,
                                                  ToneMap outputToneMap,
                                                  RollOff highlightRollOff,
                                                  unsigned int hdrPeakNits);

// Save IPP2 entire Output Transform to memory as a 3D LUT.
// The Output Transform takes the RWG/Log3G10 to the final output image via the
// user selected settings.
// Returns false if an invalid parameter was supplied. The following parameters
// are used:
// - output3DLut     : memory must be allocated before calling function. Size
// must be at least:
...
// - highlightRollOff: highlight roll off to use
// - hdrPeakNits     : only used when HDR2084 is selected
bool CreateRWGLog3G10ToOutputTransform3DLut(float *output3DLut,
                                            int edgeLength,
                                            ImageColorSpace colorSpace,
                                            ImageGammaCurve gammaCurve,
                                            ToneMap outputToneMap,
...
// Save IPP2 Output Transform to memory as a 1D LUT for the Tone Map Curve
// followed by a 3D LUT.
// The Output Transform takes the RWG/Log3G10 to the final output image via the
// user selected settings.
// Returns false if an invalid parameter was supplied. The following parameters
// are used:
// - output3DLut     : memory must be allocated before calling function. Size
// must be at least:
//                       3 * edgeLength * edgeLength * edgeLength *
//                       sizeof(float)
// - edgeLength      : must be in range 17 -- 64 inclusive
// - output1DLut     : memory must be allocated before calling function. Size
// must be at least:
//                       lutSize * sizeof(float)
// - lutSize         : entries to create in 1D Tone Curve LUT. Must be in range
// 1024 -- 65536 inclusive.
// - lutScaleFactor  : factor to scale entries in 1D Tone Curve LUT by. Must be
// 1 or higher.
// - colorSpace      : output color space to convert to from REDWideGamutRGB
// - gammaCurve      : output gamma to convert to from Log3G10
// - outputToneMap   : tone map to use
// - highlightRollOff: highlight roll off to use
// - hdrPeakNits     : only used when HDR2084 is selected
bool CreateRWGLog3G10ToOutputTransformLuts(float *output3DLut,
                                           int edgeLength,
                                           float *output1DLut,
                                           int lutSize,
                                           int lutScaleFactor,

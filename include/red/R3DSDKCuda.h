...
class EXT_CUDA_API
{
 ...
 public:
  ...
  cudaError_t(CUDA_API_CALL *cudaFreeMipmappedArray)(
      cudaMipmappedArray_t mipmappedArray);
  cudaError_t(CUDA_API_CALL *cudaHostAlloc)(void **pHost,
                                            size_t size,
                                            unsigned int flags);
  cudaError_t(CUDA_API_CALL *cudaMalloc)(void **devPtr, size_t size);
  cudaError_t(CUDA_API_CALL *cudaMalloc3D)(cudaPitchedPtr *pitchedDevPtr,
                                           cudaExtent extent);
  cudaError_t(CUDA_API_CALL *cudaMalloc3DArray)(
      cudaArray_t *array,
      const cudaChannelFormatDesc *desc,
      cudaExtent extent,
      unsigned int flags);
  cudaError_t(CUDA_API_CALL *cudaMallocArray)(cudaArray_t *array,
                                              const cudaChannelFormatDesc *desc,
                                              size_t width,
                                              size_t height,
                                              unsigned int flags);
  cudaError_t(CUDA_API_CALL *cudaMallocHost)(void **ptr, size_t size);
  cudaError_t(CUDA_API_CALL *cudaMallocMipmappedArray)(
      cudaMipmappedArray_t *mipmappedArray,
      const cudaChannelFormatDesc *desc,
      cudaExtent extent,
      unsigned int numLevels,
      unsigned int flags);
  cudaError_t(CUDA_API_CALL *cudaMallocPitch)(void **devPtr,
                                              size_t *pitch,
                                              size_t width,
                                              size_t height);
};
...
class DebayerCudaJob
{
 ...
 public:
  ...
  // This call will block until the debayer on the stream executes, if the
  // debayer has already executed no blocking will occur
  // This function must be called at a point after passing the debayerCudaJob
  // into REDCuda::processAsync
  // This will automatically be called upon destruction the DebayerCudaJob if it
  // is not manually called earlier
  void completeAsync();
...
class REDCuda
{
 public:
  ...
  Status checkCompatibility(int deviceId,
                            cudaStream_t stream,
                            cudaError_t &cuda_error);

  // processes a frame.
  // Thread safe blocking call
  // This function will enqueue the cuda kernels and wait for the processing to
  // complete
  ...
  //	Be sure you have already done setCudaDevice on the thread using the
  // passed in deviceId prior to calling process
  Status process(int deviceId,
                 cudaStream_t stream,
                 DebayerCudaJob *debayerJob,
                 cudaError_t &cuda_error);
  ...
  //		This will wait for commands enqueued by this sdk for the current
  // frame to complete and then releases any additional resources.
  //		If the commands have already completed there is no wait involved
  Status processAsync(int deviceId,
                      cudaStream_t stream,
                      DebayerCudaJob *debayerJobAsync,
                      cudaError_t &cuda_error);
